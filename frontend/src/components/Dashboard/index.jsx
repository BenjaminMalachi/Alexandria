    ...
    const fetchDashboardData = async (role, userId) => {
        ...
        if (role === 'admin') {
            ...
            try {
              ...
              // Fetch courses first
              const coursesResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/courses/teacher/courses`, {
              ...
              const homeworksResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/homework?courses=${coursesData.map(course => course._id).join(',')}`, {
                headers: { Authorization: `Bearer ${token}` },
              });
              ...
              // Fetch submissions
              const submissionsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/submission`, {
                headers: { Authorization: `Bearer ${token}` }
              });
            ...
            try {
                ...
                // FETCH courses
                const coursesResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/courses/student/courses`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                });
                ...
                //FETCH submissions
                const submissionsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/submission/student/${studentId}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                });
                const submissionsData = await submissionsResponse.json();
